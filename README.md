...
eczema_psoriasis_model.keras
Trained CNN model
requirements.txt
Python dependencies for deployment
CV20_Psoriasis_vs_Eczema_Training_Lab_10.ipynb
...
Model: MobileNetV2 pre-trained on ImageNet, used as a frozen feature extractor with a custom classification head (global average pooling → dropout 0.3 → sigmoid output), then fine-tuned on the top 30 layers of the backbone.
Training: two-phase transfer learning — 20 epochs with the backbone frozen, followed by 15 fine-tuning epochs with the top layers unfrozen, both phases using early stopping on validation loss.
Evaluation (Step 25): classification report, test accuracy, and confusion matrix, computed once on the held-out Test set.
Precision/Recall/F1 (Step 26): extracted into a table and a per-class bar chart for a clear, at-a-glance comparison.
Deployment: Streamlit Community Cloud, reading the saved .keras model to serve predictions on uploaded images.
📊 Results
...
Name
Registration Number
Asuquo, Destiny Bassey (Group Leader)
23/EG/CV/069
Johnny, Gideon Uko
23/EG/CV/079
